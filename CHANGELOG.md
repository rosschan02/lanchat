...
- 新增消息回复/引用能力：支持右键消息回复、消息内展示引用内容并可回跳原消息。
- 新增消息编辑能力：支持文本消息编辑（普通用户 2 分钟内可编辑，管理员不受时限限制）。
- 新增已读回执与未读分割线：私聊显示已读/未读，频道显示成员已读统计，并在消息列表标注未读起点。
- 新增会话偏好：支持会话置顶、免打扰和按会话草稿自动保存/恢复。
- 新增频道公告：管理员可在会话内编辑公告，频道顶部置顶展示并实时同步给成员。
- 新增个人资料修改：支持上传头像与编辑个人简介。

### 🛠 优化与修复

- 聊天消息头像增强：消息列表中展示用户上传的头像图片，未设置头像时回退显示昵称首字符。
- 扩展消息模型字段：新增 `reply_to_message_id`、`edited_at`，支持引用与编辑状态持久化。
- 新增 `chat_reads` 与 `channel_announcements` 表并自动迁移旧库，补齐已读状态与频道公告存储。
- 用户表新增 `bio` 字段并自动迁移，兼容历史数据库版本。
- 在线状态同步增强：新增 `user:profile-updated` 广播，用户头像更新可实时反映到在线列表与会话界面。
