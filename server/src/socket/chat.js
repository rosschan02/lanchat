 ...
 * 保存消息到数据库
 */
function saveMessage(fromUserId, toUserId, type, content, channelId = null) {
    const db = getDb();
    const result = db.prepare(`
    INSERT INTO messages (from_user_id, to_user_id, type, content, channel_id)
    VALUES (?, ?, ?, ?, ?)
  `).run(fromUserId, toUserId, type, content, channelId);
